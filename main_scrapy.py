...
class DataPipline:
    ...
    def close_spider(self, spider):
        ...
        with open('data/authors.json', 'w', encoding='utf-8') as fd:
...
class QuotesSpider(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        for q in response.xpath("/html//div[@class='quote']"):
            quote = q.xpath("span[@class='text']/text()").get().strip().replace('\"', "")
    ...
    def parse_author(cls, response, **kwargs):
        ...
        description = content.xpath("div[@class='author-description']/text()").get().strip().replace('\"', "'")
